...
def input_output_adder(a_bit_values, b_bit_values):
    ...
    a_bits = [f'a{i}' for i in range(num_bits)]
    b_bits = [f'b{i}' for i in range(num_bits)]
    out_bits = [f'sum{i}' for i in range(num_bits+1)]
...
if __name__ == "__main__":
    ...
    sum = convert_to_bits(int(sys.argv[1])+int(sys.argv[2]), 4)
    print("%s = %s" %(sum, output_input_adder(sum)))
#END SANITIZE ALL
